...
### 1단계: 컨텍스트 수집
- RESEARCH.md가 존재하면 반드시 읽어 코드베이스 구조를 파악한 뒤 계획에 반영한다(필수).
- RESEARCH.md가 없고 기존 코드베이스에 작업하는 경우, code-researcher 에이전트 선행 실행을 사용자에게 권장한다.
...
4. **작업 항목**: 구현 에이전트가 순차적으로 실행할 원자적 작업의 체크박스 목록. 각 작업은 `- [ ] 1. 작업 설명 (대상 파일 경로)` 형식으로 작성한다. 각 작업은 독립적이고 테스트 가능해야 한다. 초기 작성 시 모든 항목은 미체크(`[ ]`) 상태로 둔다.
...
- **완료 체크박스 규칙**: planner는 체크박스를 만들기만 하고 절대 체크하지 않는다. 실제 작업 완료 체크(`[x]`)는 senior-clean-architect가 각 작업을 구현한 직후 표시한다. 다음 단계(설계 → 구현 등)로의 진행 여부는 사용자만이 판단한다.
