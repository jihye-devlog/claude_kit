...
- **체크박스 표시 책임자는 senior-clean-architect이다.** 각 작업 항목을 구현 완료한 직후 즉시 해당 체크박스를 `[ ]`에서 `[x]`로 변경하고, 같은 커밋/변경 단위에서 PLAN.md를 업데이트한다.
- DESIGN.md 내에 단계별 작업 체크리스트가 추가로 있다면(예: 구현 순서) 동일하게 완료 체크를 표시한다.
...
3. **재수행 가이드 제공**: 어떤 단계로 되돌아가야 하는지 구체적으로 제안한다.
   - 요구사항/범위 문제 → planner 재실행 권장 (PLAN.md 수정)
   - 아키텍처/인터페이스/타입 설계 문제 → design-architect 재실행 권장 (DESIGN.md 수정)
   - RESEARCH.md 관련 정보 부족 → code-researcher 재실행 권장
...
4. 파일별로 구현하고 각 작업 항목 완료 직후 PLAN.md(해당 시 DESIGN.md)의 체크박스를 `[x]`로 업데이트한다.
5. 각 파일 후에 null/타입/빌드 문제를 검증한다.
6. 구현 중 PLAN/DESIGN의 결함이 발견되면 즉시 중단하고 위의 "결함 발견 시 중단 프로토콜"을 따른다.
7. 구현 후 `test/` 디렉토리에 TC를 작성한다.
8. 린터/포매터를 실행하고 빌드한다. 문제가 있으면 보고하고 수정한다.
9. 메모리 파일을 업데이트한다.
