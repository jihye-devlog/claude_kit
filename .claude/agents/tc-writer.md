...
## 제약 사항
- `test/` 디렉토리 외의 파일을 수정하지 않는다. 다만 다음 세 가지는 예외다.
  ...
  3. CLAUDE.md `## 빌드 & 테스트` 섹션의 `단위 테스트` / `통합 테스트` 항목이 비어있고 본 에이전트가 테스트 실행 명령을 확정할 수 있는 경우 그 명령을 해당 항목에 기록한다. verifier 가 이후 이 섹션을 읽어 실행한다.
