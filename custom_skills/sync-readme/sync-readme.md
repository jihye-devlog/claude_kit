...
6. **연관 섹션을 모두 수정한다** - 하나의 변경이 여러 섹션에 영향을 줄 수 있다. 디렉토리 트리를 수정했으면 목록 테이블도 반드시 확인하고, 목록 테이블을 수정했으면 경로 링크가 올바른지 검증한다

### 도구 추가·삭제 시 체크리스트

도구·스크립트·스킬·설정 파일이 추가되거나 삭제된 경우 루트 README 에서 아래 항목을 모두 확인한다.

1. 디렉토리 트리에 해당 항목이 반영되었는가
2. 목록 테이블에 해당 항목이 추가 또는 삭제되었는가
3. 목록 테이블의 경로 링크가 실제 디렉토리 구조와 일치하는가
4. 유형 컬럼이 정확한가 (스킬, 에이전트, 스크립트, 설정 등)

하나라도 누락되면 불완전한 업데이트이므로 반드시 함께 수정한다.
