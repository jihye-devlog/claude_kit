    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "prompt",
            "prompt": "파일이 변경되었습니다. 변경된 파일이 README.md 자체가 아니라면, 프로젝트 루트 README.md와 관련 하위 README.md가 이 변경을 반영하고 있는지 확인하세요. 반영되지 않았다면 반드시 skill-executor 서브에이전트로 sync-readme 스킬을 실행하세요. README.md만 변경된 경우에는 실행하지 마세요."
          }
        ]
      }
    ]
