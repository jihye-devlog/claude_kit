    ...
    "Stop": [
      {
        "hooks": [
          {
            "type": "prompt",
            "prompt": "이번 턴에 Claude 가 직접 Edit/Write 로 편집·생성한 파일이 있는지 먼저 확인하세요. 편집된 파일이 전혀 없다면(단순 질문·설명·조회 응답 등) 아무 보고도 하지 말고 즉시 종료를 허용하세요. 편집된 파일이 있고 그것이 README.md 자체만이 아니라면, 프로젝트 루트 README.md와 관련 하위 README.md가 변경을 반영하는지 점검하고, 반영되지 않았다면 skill-executor 서브에이전트로 sync-readme 스킬을 실행하세요. 이미 이번 턴에 sync-readme 가 실행되었거나 README.md 만 변경된 경우에는 실행하지 말고 종료를 허용하세요."
          }
        ]
      }
    ]
