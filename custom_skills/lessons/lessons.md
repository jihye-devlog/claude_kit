---
name: lessons
description: Record mistakes Claude makes and prevent repetition. ALWAYS invoke this skill when: (1) the user corrects Claude's output or approach ("아니야", "틀렸어", "잘못됐어", "그게 아니라", "다시", "수정해", "왜 ~안했어", "~없는데", "~안되었어", "~잘못", "~아닌데"), (2) a command, build, or file operation fails due to Claude's error, (3) Claude realizes mid-task that a previous step was wrong, (4) the user runs /lessons, or (5) Claude corrects its own mistake after user points it out — invoke IMMEDIATELY after the fix, not after being reminded. CRITICAL: Do NOT wait for the user to ask "why didn't you run lessons?" — the skill must fire right after any correction is applied.
---
...
| 상황 | 판단 기준 |
|------|-----------|
| 사용자 교정 | "아니야", "틀렸어", "그게 아니라", "다시", "수정해", "왜 ~안했어", "~없는데", "~안되었어", "~잘못", "~아닌데" 등 정정 발언 |
| 명령/빌드 실패 | Claude가 작성한 코드나 명령이 에러를 유발한 경우 |
| 자기 인식 | 작업 도중 이전 단계가 잘못됐음을 스스로 발견한 경우 |
| 실수 교정 직후 | Claude가 실수를 인지하고 교정한 직후 즉시 발동 (사용자가 재촉하기 전에 먼저 실행) |
