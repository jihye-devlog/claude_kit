...
```bash
cp add-tool/add-tool.md ~/.claude/commands/add-tool.md
```
...
```
/add-tool save-progress ~/.claude/commands/save-progress.md
...
| 유형 | 글로벌 설치 경로 | 활성화 방법 |
|------|----------------|-------------|
| 스킬 (`.md`) | `~/.claude/commands/` | Claude Code 재시작 후 `/<명령어>` 로 사용 |
